...
export type HubGuide = {
  title: string
  /**
   * 문서 URL의 마지막 조각. 영문 소문자·숫자·하이픈만.
   *
   * 왜 제목에서 자동 생성하지 않는가: 생성 함수의 docSlug()는 한글 제목에서 ASCII만 남기고
   * 해시를 붙인다. 그래서 '폴드8 멀티윈도우 3분할 설정하는 법'이 '8-3-dw76cz'가 됐다.
   * 검색 결과에 그대로 노출되는 자리인데 무슨 글인지 읽을 수 없고, 제목을 조금만 고쳐도
   * 해시가 달라져 URL이 통째로 바뀐다. 사람이 정한 값을 여기 적어 두면 둘 다 해결된다.
   *
   * 비워 두면 종전대로 docSlug()가 만든 값이 쓰인다 — 아직 손대지 않은 허브를 위한 폴백이다.
   */
  slug?: string
  /**
   * 이 문서가 다룰 범위와 다루지 않을 범위. 생성 프롬프트에 그대로 들어간다(2026-08-12 추가).
   *
   * 왜 필요한가: 제목이 비슷한 문서가 한 허브에 여러 개 생기면 모델이 같은 내용을 반복해서
   * 쓴다. 그러면 서로 순위를 갉아먹는다(키워드 카니벌라이제이션) — 특히 이미 검색 성과가
   * 나고 있는 문서가 있을 때 손해가 크다. 제목만으로 경계가 분명하지 않은 항목에는 이 값으로
   * "여기까지가 이 문서, 그 밖은 다른 문서"를 못 박는다. 경계가 자명한 항목은 비워도 된다.
   */
  intent?: string
  /**
   * 이미 확인이 끝난 사실. 생성 프롬프트가 "그대로 쓰라"고 지시받는 값이다(2026-08-12 추가).
   *
   * 왜 필요한가: 생성 규칙 1번이 "모르는 것은 쓰지 마라"인데, 모델은 이 규칙을 넓게 적용해
   * **사람이 확인해서 넘겨준 수치까지** 빼버린다. 실측(2026-08-12, 멜론 허브): intent에 공식
   * 이용권 가격 네 개를 적어 줬는데도 '무제한 요금제 비교' 문서에 금액이 한 줄도 들어가지
   * 않았다. 요금제 비교에 금액이 없으면 그 문서는 존재 이유가 없다.
   *
   * intent(범위)와 다른 축이다 — intent는 "무엇을 다루는가", facts는 "무엇이 이미 참인가".
   * 여기 적는 값은 반드시 출처를 확인한 것이어야 하고, 확인 날짜를 문장에 함께 적는다.
   * 바뀔 수 있는 값(가격·정책)은 "확인 시점" 표기가 곧 정확성이다.
   */
  facts?: string[]
...
export type HubAffiliateSlot = {
  ...
  network: 'coupang'
  /**
   * 상품의 성격. 문서 페이지가 어떤 슬롯을 보여줄지 이 값으로 고른다.
   *
   * 왜 필요한가: 사용법·문제해결 문서를 보는 독자는 이미 기기를 가지고 있다. 그 사람에게
   * 본체를 파는 링크는 의미가 없고, 필요한 것은 케이스·필름·충전기다. 반대로 비교·구매
   * 문서를 보는 독자는 아직 사지 않았으므로 본체가 먼저다.
   *
   * slot 이름의 접미사('-body' 등)로 추측하지 않고 명시적 필드를 두는 이유: 이름 규칙은
   * 허브마다 어긋난다(실제로 robot-vacuum-2026은 본체 슬롯이 'rv26-roborock'·'rv26-dreame'라
   * '-body'가 없다). 이름으로 판별하면 그런 허브에서 조용히 틀린다.
   */
