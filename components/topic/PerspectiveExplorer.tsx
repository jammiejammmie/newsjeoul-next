...
export default function PerspectiveExplorer({ blocks }: { blocks: Block[] }) {
  ...
  return (
    <section style={{ marginBottom: 28 }}>
      ...
      <div key={activeIdx} style={{ borderTop: '1px solid var(--border)', paddingTop: 20 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 12 }}>
          {active.axis}
        </div>
        <p style={{ fontSize: 15, fontWeight: 500, color: 'var(--text2)', lineHeight: 1.85 }}>{active.content}</p>
      </div>
    </section>
  )
}
