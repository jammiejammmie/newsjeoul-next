...
| DEC-009 | Threads 배급 전면 개편 — 비율 강제 → 트리거 기반, 링크를 첫 댓글로 이동, 시간대별 유형 분리, 에버그린 30~40% | 해당 없음 → PM 전면 개편 지시 | 2026-08-12 |
...
### DEC-009 — Threads 배급 전면 개편: 비율 강제 → 트리거, 링크 → 첫 댓글, 시간대별 유형 분리, 에버그린 30~40%

| 출처 | 내용 |
|---|---|
| **디자인팀 제안** | 해당 없음 |
| **개발팀 제안** | 해당 없음(현황 실측만 보고 — 허브 13개·허브 문서 213건이 어떤 채널로도 나가지 않고 있었고, `evergreen_queue` 36건이 전부 skipped라 신규 허브 자동 생성은 0건이었다) |
| **PM 최종 결정** | (1) **비율 강제 제거 → 트리거 기반 전환** — 허브 키워드가 포함된 뉴스 토픽이 감지되면 해당 허브 링크 포스팅을 자동 트리거하고, 새 허브 문서가 생성되면 에버그린 포스팅을 자동 트리거한다. 뉴스는 기본으로 계속 유지. (2) **에버그린 비중 상향** — 하루 게시 중 에버그린 가이드 30~40%, 클릭률이 높은 사용법·설정법 문서 우선. (3) **링크를 댓글로 이동** — 본문은 링크 없이 읽어도 완결이어야 하고, 링크는 첫 번째 댓글에 자동으로 붙인다. 본문 500자 예산에 여유가 생기는 만큼 내용을 더 풍부하게. (4) **시간대 최적화** — 뉴스·이슈는 오전 7시~오후 2시, 에버그린 가이드는 오후 6시~밤 11시에 집중(기존 30분 균일 배포에서 시간대별 유형 분리로). (5) **제품 이슈 연결** — 갤럭시·노트북·가전 관련 뉴스 토픽이 감지되면 해당 허브를 자동 연결하되, 판단 기준은 토픽 제목의 허브 키워드 포함 여부이고 관련성 점수가 낮으면 어색한 연결을 만들지 말고 일반 뉴스로 발행한다 |

- **결정 이유(가장 중요)**: 링크가 본문 끝에 붙어 있던 구조는 URL만 167~209자를 먹어 본문 예산을 260자까지 밀어냈다 — 완결형 글을 요구하면서 그 예산은 주지 않는 모순이었다. 링크를 댓글로 내리면 본문 예산이 500자로 늘고, "링크를 누르지 않아도 읽을 가치가 있는 글"이라는 2026-07-29 개편의 원래 목표가 비로소 성립한다. 유형·시간대를 가르는 이유는 독자의 상태가 다르기 때문이다: 뉴스 독자는 "무슨 일이 있었나"를 모르는 상태고(오전), 가이드 독자는 이미 문제를 겪고 있다(저녁 — PM 예시 "폴드8 배터리 하루 못 가는 분들, 이 설정 먼저 확인하세요"). 비율을 강제하지 않고 트리거로 두는 이유는, 고정 비율이 "올릴 것이 없을 때도 억지로 올리게" 만들기 때문이다.
- **영향 문서·섹션**: `netlify/functions/threads-strategy.js`(신설 — 유형 선택·허브 연결 판정·에버그린 우선순위·댓글 조립의 순수 로직), `netlify/functions/post-threads-background.js`(선택 계층을 topics 단일에서 topics+hub_documents 이중으로 확장, 링크 댓글 부착, 유형별 집계), `app/hub-targets.json/route.ts`(허브 `newsKeywords`·`newsExclude` 노출 — 함수가 허브 연결을 판정할 수 있게), `scripts/test-post-threads.js`(회귀 97건 — 링크 위치 계약이 바뀐 5건 재작성 + 개편 검증 19건 신설). **DB 마이그레이션 없음**: 에버그린 dedup 정본은 `threads_posts.source_url`, 유형 집계는 2026-07-29 개편으로 비어 있던 `hook_type` 컬럼을 재사용한다.
- **의도적으로 하지 않은 것**: 카테고리 배분(`categoryAllocation`, 가중치 0.20)은 그대로 뒀다. 이번 지시의 "비율 강제 제거"는 뉴스/에버그린 **유형 사이**의 강제 배분을 가리키고, 카테고리 배분은 뉴스 **내부**의 다양성 장치라 성격이 다르다 — 없애면 한 분야가 하루를 독점한다.
